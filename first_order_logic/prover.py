...
class Prover:
    ...
    Attributes:
        _assumptions (`~typing.FrozenSet`\\[`~first_order_logic.proofs.Schema`]): the
            assumptions/axioms of the proof being created, which include
            `AXIOMS`.
        _lines (`~typing.List`\\[`~first_order_logic.proofs.Proof.Line`]): the current
    ...
    def add_universal_instantiation(self, instantiation: Union[Formula, str],
        ...
        # Task 10.1
        predicate = quantified.predicate
        var = quantified.variable
        predicate = predicate.substitute({var: Term('_')})
        step1 = self.add_instantiated_assumption(Formula('->', quantified, instantiation), Prover.UI,
                                                 {'R': predicate, 'x': var, 'c': term})
    ...
    def add_tautological_implication(self, implication: Union[Formula, str],
        ...
        for line in reversed(line_numbers):
            formula = Formula('->', self._lines[line].formula, formula)
        step = self.add_tautology(formula)
        i = 0
        while formula.second != implication:
            step = self.add_mp(formula.second, line_numbers[i], step)
            i += 1
    ...
    def add_existential_derivation(self, consequent: Union[Formula, str],
        ...
        # Task 10.3
        v = quantified.variable
        step1 = self.add_ug(Formula('A', v, self._lines[line_number2].formula), line_number2)
        step2 = self.add_instantiated_assumption('((A' + v + '[' + str(conditional) + ']&' + str(quantified) + ')->' + str(consequent) + ')', Prover.ES,
                                         {'R': quantified.predicate.substitute({v: Term('_')}), 'x': v, 'Q': consequent})
    ...
    def add_flipped_equality(self, flipped: Union[Formula, str],
        ...
        assert equality == Formula('=', [flipped.arguments[1],
                                         flipped.arguments[0]])
        # Task 10.6
        mid = next(fresh_variable_name_generator)
        term1, term2 = equality.arguments[0], equality.arguments[1]
        term1_eq_mid = Formula('=', (term1, Term(mid)))
        term2_eq_mid = Formula('=', (term2, Term(mid)))
        term1_eq_term1 = Formula('=', (term1, term1))
        template = Formula('=', (Term('_'), Term(mid)))
        step1 = self.add_instantiated_assumption(Formula('->', equality, Formula('->', term1_eq_mid, term2_eq_mid)), Prover.ME,
                                                 {'c': term1, 'd': term2, 'R': template})
        step2 = self.add_mp(Formula('->', term1_eq_mid, term2_eq_mid), line_number, step1)
        step3 = self.add_ug(Formula('A', mid, Formula('->', term1_eq_mid, term2_eq_mid)), step2)
        step4 = self.add_universal_instantiation(Formula('->', term1_eq_term1, flipped), step3, term1)
        step5 = self.add_instantiated_assumption(term1_eq_term1, Prover.RX, {'c': term1})
        return self.add_mp(flipped, step5, step4)
    ...
    def add_free_instantiation(self, instantiation: Union[Formula, str],
        ...
        # Task 10.7
        step = line_number
        vars_set = self._lines[step].formula.free_variables()
        sub_to_z = dict()
        sub_from_z = dict()
        for var in substitution_map.keys():
            if var in vars_set:
                new_z = next(fresh_variable_name_generator)
                sub_to_z[var] = Term(new_z)
                sub_from_z[new_z] = substitution_map[var]
        for var in sub_to_z.keys():
            pred = self._lines[step].formula
            step = self.add_ug(Formula('A', var, pred), step)
            step = self.add_universal_instantiation(pred.substitute({var: sub_to_z[var]}), step, sub_to_z[var])
        for var in sub_from_z.keys():
            pred = self._lines[step].formula
            step = self.add_ug(Formula('A', var, pred), step)
            step = self.add_universal_instantiation(pred.substitute({var: sub_from_z[var]}), step, sub_from_z[var])
        return step
    ...
    def add_substituted_equality(self, substituted: Union[Formula, str],
        ...
        # Task 10.8
        left = parametrized_term.substitute({'_': equality.arguments[0]})
        new_RX = Formula('=', [left, left])
        step1 = self.add_instantiated_assumption(new_RX, self.RX, {'c': left})
        step2 = self.add_instantiated_assumption(Formula('->', equality, Formula('->', new_RX, substituted)), self.ME,
                                                 {'R': Formula('=', [left, parametrized_term]),
                                                  'c': equality.arguments[0], 'd': equality.arguments[1]})
        step3 = self.add_mp(Formula('->', new_RX, substituted), line_number, step2)
        return self.add_mp(substituted, step1, step3)
    ...
    def _add_chaining_of_two_equalities(self, line_number1: int,
        ...
        # Task 10.9.1
        new_equality = Formula('=', [equality1.arguments[0], equality2.arguments[1]])
        step1 = self.add_instantiated_assumption(Formula('->', equality2, Formula('->', equality1, new_equality)), self.ME,
                                                 {'R': Formula('=', [equality1.arguments[0], Term('_')]),
                                                  'c': equality1.arguments[1], 'd': equality2.arguments[1]})
        step2 = self.add_mp(Formula('->', equality1, new_equality), line_number2, step1)
        return self.add_mp(new_equality, line_number1, step2)
    ...
    def add_chained_equality(self, chained: Union[Formula, str],
        ...
        # Task 10.9.2
        step = line_numbers[0]
        for line in range(1, len(line_numbers)):
            step = self._add_chaining_of_two_equalities(step, line_numbers[line])
        return step
