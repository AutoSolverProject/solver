...
# File name: first_order_logic/semantics.py
...
class Model(Generic[T]):
    ...
    def evaluate_formula(self, formula: Formula,
        ...
        if is_unary(root):
            ...
            for arg in formula.arguments:
                args.append(self.evaluate_term(arg, assignment))
            return tuple(args) in self.relation_meanings[root]
        elif is_equality(root):
            args = formula.arguments
            return self.evaluate_term(args[0], assignment) == self.evaluate_term(args[1], assignment)
        for elem in self.universe:
            new_assignment = assignment.copy()
            new_assignment[formula.variable] = elem
            val = self.evaluate_formula(formula.predicate, new_assignment)
            if val and root == 'E':
                return True
            elif not val and root == 'A':
                return False
        return root == 'A'
        ...
        for formula in formulas:
            ...
            for relation,arity in formula.relations():
                assert relation in self.relation_meanings and \
                       self.relation_arities[relation] in {-1, arity}
        # Task 7.9
            for var in list(formula.free_variables()):
    ...
    def eval_formula_helper(self, formula: Formula, assignment: Dict[str, T] = frozendict()) \
            -> bool:
        if is_unary(formula.root):  # this is of type ~p
            return OPERATOR_BOOLEAN_DICT[formula.root](
                self.eval_formula_helper(formula.first, assignment))
        if is_equality(formula.root):  # this is of type p = q
            return self.evaluate_term(formula.arguments[0], assignment) == \
                   self.evaluate_term(formula.arguments[1], assignment)
        if is_relation(formula.root):  # this is of type p(a,b,c(
            tup = tuple(self.evaluate_term(argument, assignment)
                        for argument in formula.arguments)
            return tup in self.relation_meanings[formula.root]
        if is_binary(formula.root):  # this is like p op q
            return OPERATOR_BOOLEAN_DICT[formula.root](
                self.eval_formula_helper(formula.first, assignment),
                self.eval_formula_helper(formula.second, assignment))
        # this is a quantifier, i.e. Ax[p] or Ex[p]
        new_assignment = {key: assignment[key] for key in assignment.keys()}
        if formula.root == 'A':  # check all
            for option in self.universe:
                new_assignment[formula.variable] = option
                if not self.eval_formula_helper(formula.predicate,
                                                new_assignment):
                    return False
            return True
        elif formula.root == 'E':  # exists one
            for option in self.universe:
                new_assignment[formula.variable] = option
                if self.eval_formula_helper(formula.predicate, new_assignment):
                    return True
            return False
