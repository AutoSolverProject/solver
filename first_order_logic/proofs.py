...
class Schema:
    ...
    Attributes:
        formula (`~first_order_logic.syntax.Formula`): the formula of the schema.
    ...
    def _instantiate_helper(formula: Formula,
        ...
        Examples:
            ...
            Traceback (most recent call last):
              ...
            first_order_logic.proofs.Schema.BoundVariableError: ('y', 'Q')
        ...
        if is_equality(root):
            ...
            first = Schema._instantiate_helper(formula.first, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables)
            if is_binary(root):
            ...
            if formula.variable in constants_and_variables_instantiation_map.keys():
                new_var = str(constants_and_variables_instantiation_map[formula.variable])
                new_formula = formula.predicate.substitute({formula.variable: Term(new_var)})
                bound_variables = set(bound_variables)
                bound_variables.add(new_var)
                cons_var = dict(constants_and_variables_instantiation_map)
                cons_var.pop(new_var, "unnecessary")
                return Formula(root, new_var,
                               Schema._instantiate_helper(new_formula, frozendict(cons_var),
                                                          relations_instantiation_map, frozenset(bound_variables)))
            bound_variables = set(bound_variables)
            bound_variables.add(formula.variable)
            return Formula(root, formula.variable, Schema._instantiate_helper(formula.predicate, constants_and_variables_instantiation_map,
                                                          relations_instantiation_map, frozenset(bound_variables)))
        else:
            try:
                formula = formula.substitute(constants_and_variables_instantiation_map, bound_variables)
            ...
            if root in relations_instantiation_map:
                vars_list = relations_instantiation_map[root].free_variables()
                ...
                if len(args) == 1:
                    sub['_'] = args[0]
                try:
                    formula = relations_instantiation_map[root].substitute(sub, bound_variables)
                except ForbiddenVariableError as e:
                    raise Schema.BoundVariableError(e.variable_name, root)
                if '_' not in relations_instantiation_map[root].constants():
                    vrs = formula.free_variables()
                    for var in bound_variables:
                        if var in vrs:
                            raise Schema.BoundVariableError(var, root)
                elif is_quantifier(formula.root):
                    if Term(formula.variable) in sub.values():
                        raise Schema.BoundVariableError(formula.variable, root)
                return formula
    ...
    def instantiate(self, instantiation_map: InstantiationMap) -> \
        ...
        for key in instantiation_map.keys():
            if key not in self.templates:
                return None
            if is_constant(key):
                cons_var[key] = instantiation_map[key]
            elif is_variable(key):
                cons_var[key] = Term(instantiation_map[key])
            else:
                relations[key] = instantiation_map[key]
        formula = self.formula
        try:
            return Schema._instantiate_helper(formula, cons_var, relations, frozenset())
        except self.BoundVariableError:
            return None
...
class Proof:
    ...
    Attributes:
        assumptions (`~typing.FrozenSet`\\[`Schema`]): the assumption/axioms of
            the proof.
        conclusion (`~first_order_logic.syntax.Formula`): the conclusion of the proof.
    ...
    class AssumptionLine:
        ...
        Attributes:
            formula (`~first_order_logic.syntax.Formula`): the formula justified by the
                line.
            assumption (`Schema`): the assumption/axiom that instantiates the
                formula.
            instantiation_map (`~typing.Mapping`\\[`str`, `~typing.Union`\\[`~first_order_logic.syntax.Term`, `str`, `~first_order_logic.syntax.Formula`]]):
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            # Task 9.5
            assLine = lines[line_number]
            return assLine.assumption in assumptions and \
                   assLine.assumption.instantiate(assLine.instantiation_map) == assLine.formula
    ...
    class MPLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            assert line_number < len(lines) and lines[line_number] is self
            # Task 9.6
            mpLine = lines[line_number]
            consequent = mpLine.formula
            antecedent = lines[mpLine.antecedent_line_number].formula
            conditional = lines[mpLine.conditional_line_number].formula
            con_line = mpLine.conditional_line_number
            ant_line = mpLine.antecedent_line_number
            return con_line < line_number and ant_line < line_number and \
    ...
    class UGLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            assert line_number < len(lines) and lines[line_number] is self
            # Task 9.7
            ugLine = lines[line_number]
            formula = ugLine.formula
            if not (0 <= ugLine.predicate_line_number < line_number and is_quantifier(formula.root)):
                return False

            predicate = lines[ugLine.predicate_line_number].formula

            var = formula.variable
            return formula == Formula('A', var, predicate)
...
def axiom_specialization_map_to_schema_instantiation_map(
    ...
    Parameters:
        propositional_specialization_map: map specifying how some propositional
            ...
            `~propositional_logic.axiomatic_systems.AXIOMATIC_SYSTEM` specializes into
    ...
    # Task 9.11.1
    sub_map = dict()
    for s, f in propositional_specialization_map.items():
        sub_map[s.upper()] = Formula.from_propositional_skeleton(f, substitution_map)
    return sub_map
...
def prove_from_skeleton_proof(formula: Formula,
    ...
    for line in skeleton_proof.lines:
        line_formula = Formula.from_propositional_skeleton(line.formula, substitution_map)
        if line.rule == MP:
            lines.append(Proof.MPLine(line_formula, line.assumptions[0], line.assumptions[1]))
        else:
            rule = line.rule
            if rule == I0:
                schema = I0_SCHEMA
                ...
                schema = R_SCHEMA
            sub_map = axiom_specialization_map_to_schema_instantiation_map(PropositionalInferenceRule.formula_specialization_map(rule.conclusion, line.formula), substitution_map)
            lines.append(Proof.AssumptionLine(line_formula, schema, sub_map))
...
def prove_tautology(tautology: Formula) -> Proof:
    ...
    return prove_from_skeleton_proof(tautology, propositional_tautology_proof, sub_map)
