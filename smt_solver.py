    ...
    while state != UNSAT:
        ...
        if state == SAT and congruence_closure_unviolated and len(model_over_skeleton.keys()) == len(skeleton.variables()):
            ...
            state, model_over_skeleton, updated_skeleton = \
                sat_solver(updated_skeleton, partial_model=model_over_skeleton, conflict=conflict, max_rounds=1)
            ...
            state, model_over_skeleton, updated_skeleton = sat_solver(updated_skeleton, partial_model=model_over_skeleton, max_rounds=1)
